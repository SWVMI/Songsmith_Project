...
export default function Explore() {
  ...
  useEffect(() => {
    ...
    API.get('/posts')
      ...
      .catch(() => { if (!cancelled) setErrorMsg('Failed to load collaboration posts. Please try again.'); })
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-6 py-16 font-mono text-xs">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 border-b border-zinc-800 pb-6 gap-4">
        <div>
        ...
        <Link to="/create-post" className="bg-amber-400 hover:bg-amber-300 text-zinc-950 px-5 py-3 font-bold uppercase tracking-wider transition flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-16 text-center text-zinc-500 font-mono space-y-3">
          <p className="text-sm uppercase tracking-wider">No collaboration opportunities available yet.</p>
          <p className="text-xs text-zinc-600">Publish a requirement above to start receiving applications.</p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-400 mb-6 line-clamp-3">{p.description}</p>
                  <p className="text-[11px] text-zinc-500 mb-4">Posted by {p.creator?.username || 'Unknown'}</p>
